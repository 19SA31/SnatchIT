...
const deleteSelectedCoupon = (couponId, message) => {
  return new Promise(async (resolve, reject) => {
    if (message === "unlist") {
      let result = await couponModel.updateOne({ _id: couponId }, { $set: { isListed: false } });
      resolve(result);
    } else {
      let result = await couponModel.updateOne({ _id: couponId }, { $set: { isListed: true } });
      resolve(result);
    }
  });
};
